/**     * Interface构造器     * @param {String} name 接口名     * @param {Array} methods 接口中的抽象方法          * @exception {Error} 参数不合格时抛出异常     * @example     *      var IActionListener = new Interface("IActionListener",["method1","method2"]);     */
 ...
 /**     * 接口实现的检查，保证object是InterfaceN的实例     * @param {Object} oInterface 待检查的接口实例     * @param {Class} InterfaceN 被实现的Interface     * @exception {Error} 参数不合格时抛出异常     * @example      *      Interface.ensureImplements(oActionListener,IActionListener);     */
...
/** * 接口的实现 * @param {function} ImplementsClass 待实现的类 * @param {object} InterfaceN 被实现的Interface，Interface的实例 * @exception {Error} 参数不合格时抛出异常 * @example  *      implements(ActionListener,IActionListener); */
...
//创建实现类var ActionListener = function(){};//实现implements(ActionListener,IActionListener);//这个时候，ActionListener.prototype已经是如下这个样子了：/*    ActionListener.prototype = {        method1 : function(){},        method2 : function(){}    };*///接下来可以真正的填充被空实现的方法的逻辑了ActionListener.prototype = {
    method1 : function(){
        alert("这是方法1");
    },
    method2 : function(){
